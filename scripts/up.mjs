...
async function main() {
  console.log(`[start:up] starting web coordinator at ${server}`);
  const web = runNpm(['run', 'start:web'], 'web');

  try {
    await waitForCoordinator(server);
    console.log('[start:up] coordinator ready');
    await seed(server);
    console.log('[start:up] starting worker loop');

    const worker = runNpm(
      ['run', 'start', '-w', '@opencause/worker', '--', 'loop', '--server', server, '--interval-ms', intervalMs],
      'worker'
    );
...
main().catch((error) => {
  console.error(`[start:up] ${error.message}`);
  process.exit(1);
});
