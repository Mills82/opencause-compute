...
export async function upsertProjectCorpusEstimateRelational(input: Omit<ProjectCorpusEstimate, 'id' | 'refreshedAt' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<ProjectCorpusEstimate | undefined> {
  ...
  await getPool().query(`CREATE TABLE IF NOT EXISTS project_corpus_estimates (id UUID PRIMARY KEY, project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE, corpus_source TEXT NOT NULL, query TEXT NOT NULL, eligible_document_count INTEGER NOT NULL, ingested_document_count INTEGER NOT NULL, packets_created_from_ingested_documents INTEGER NOT NULL, average_packets_per_document NUMERIC NOT NULL, estimated_total_packets INTEGER NOT NULL, estimate_method TEXT NOT NULL, refresh_status TEXT NOT NULL, failure_reason TEXT, refreshed_at TIMESTAMPTZ NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
  await getPool().query(`CREATE UNIQUE INDEX IF NOT EXISTS project_corpus_estimates_project_source_query_idx ON project_corpus_estimates(project_id, corpus_source, query)`);
...
export async function ingestSourcesRelational(input: { projectSlug: string; projectName: string; projectDescription: string; sources: IngestSource[]; extractor?: 'local-llm-v2' }): Promise<{ project: Project; packetsCreated: number; packetsSkipped: number } | undefined> {
  if (!enabled()) return undefined;
  const client = await getPool().connect();
  try {
    await client.query('BEGIN');
