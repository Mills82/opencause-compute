...
describePg('real Postgres relational storage integration', () => {
  ...
  it('supports cron-style ingestion semantics without clobbering existing claims/results', async () => {
    ...
    try {
      ...
      await pool.query("INSERT INTO extraction_results(id,work_packet_id,node_id,claim_id,extractor_version,result_hash,summary,validated,format_validated,consensus_status,review_status,validation_errors,warnings,submitted_at,provenance) VALUES(gen_random_uuid(),$1,$2,$3,'v','h','ok',true,true,'consensus_pending','not_reviewed','[]','[]',NOW(),'{}')", [claimedPacket.id, node.rows[0].id, claim.id]);
  ...
  it('supports volunteer profile setup read/update/team changes without clobbering worker state', async () => {
    ...
    try {
      ...
      await pool.query("INSERT INTO extraction_results(id,work_packet_id,node_id,claim_id,extractor_version,result_hash,summary,validated,format_validated,consensus_status,review_status,validation_errors,warnings,submitted_at,provenance) VALUES(gen_random_uuid(),$1,$2,$3,'v','h','ok',true,true,'consensus_pending','not_reviewed','[]','[]',NOW(),'{}')", [packet.id, registration.node.id, claim.id]);
