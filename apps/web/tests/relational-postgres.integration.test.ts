...
async function expectRequiredRelationalTables(pool: Pool) {
  const required = ['projects', 'work_packets', 'volunteer_nodes', 'work_claims', 'extraction_results', 'extracted_claims', 'worker_control', 'ingestion_runs', 'audit_events'];
...
describePg('real Postgres relational storage integration', () => {
  ...
  it('supports cron-style ingestion semantics without clobbering existing claims/results', async () => {
    ...
    try {
      ...
      await pool.query("INSERT INTO extraction_results(id,work_packet_id,node_id,claim_id,extractor_version,result_hash,summary,validated,format_validated,consensus_status,review_status,validation_errors,warnings,submitted_at,provenance) VALUES(gen_random_uuid(),$1,$2,$3,'Local LLM v2','h','ok',true,true,'consensus_pending','not_reviewed','[]','[]',NOW(),jsonb_build_object('workerVersion','0.1.0','extractorVersion','Local LLM v2','promptVersion','test','promptHash','hash','packetSchemaVersion','work-packet-v1','extractionTimestamp',NOW()::text,'workerPlatform','linux','workerCapabilities',jsonb_build_array(),'resultValidationVersion','claims-v2'))", [claimedPacket.id, node.rows[0].id, claim.id]);
  ...
  it('supports volunteer profile setup read/update/team changes without clobbering worker state', async () => {
    ...
    try {
      ...
      await pool.query("INSERT INTO extraction_results(id,work_packet_id,node_id,claim_id,extractor_version,result_hash,summary,validated,format_validated,consensus_status,review_status,validation_errors,warnings,submitted_at,provenance) VALUES(gen_random_uuid(),$1,$2,$3,'Local LLM v2','h','ok',true,true,'consensus_pending','not_reviewed','[]','[]',NOW(),jsonb_build_object('workerVersion','0.1.0','extractorVersion','Local LLM v2','promptVersion','test','promptHash','hash','packetSchemaVersion','work-packet-v1','extractionTimestamp',NOW()::text,'workerPlatform','linux','workerCapabilities',jsonb_build_array(),'resultValidationVersion','claims-v2'))", [packet.id, registration.node.id, claim.id]);
