...
import { buildPubMedCorpusEstimateInput, CKM_PROJECT_SLUG, DEFAULT_CKM_QUERY, upsertProjectCorpusEstimate } from '../../../../../lib/project-progress/refresh';
...
function queryFromEnv() {
  return process.env.CKM_CORPUS_QUERY ?? process.env.CRON_PUBMED_QUERY ?? DEFAULT_CKM_QUERY;
}
...
async function refreshProjectProgress() {
  ...
  const input = await buildPubMedCorpusEstimateInput({ query, db, email: process.env.NCBI_EMAIL, apiKey: process.env.NCBI_API_KEY });
