        ...
        <article className="rounded-xl border border-line bg-panel p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-300">Prototype desktop app for selected beta testing.</p>
            </div>
            {isPrototype ? <span className="w-fit rounded-full border border-yellow-500/60 px-3 py-1 text-xs font-semibold text-yellow-100">Prototype</span> : null}
          </div>
          {windowsUrl ? (
            <div className="mt-5 space-y-4 text-sm text-slate-300">
              {isPrototype ? (
                <div className="rounded border border-yellow-500/50 bg-yellow-500/10 p-3 text-yellow-100">
                  This is a prototype build for trusted QA. Windows may show SmartScreen warnings until code signing is configured.
                </div>
              ) : null}
